...
/****** Object:  StoredProcedure [dbo].[SP_GetAllProducts]    Script Date: 16/3/2023 4:59:58 AM ******/
...
AS
	...
	IF @keyword = ''
	BEGIN
		SELECT @pages = COUNT(CPRO_ID)
		FROM dbo.CVD_PRODUCT
		WHERE CPRO_DELETIONSTATE = 0
	END
	...
	BEGIN
		...
		FROM dbo.CVD_PRODUCT
		WHERE CPRO_DELETIONSTATE = 0
		AND (@filterType = 'Name' AND @keyword != '' AND  CPRO_TITLE like '%' + @keyword + '%')
	END
	...
	BEGIN
		...
		SELECT * FROM 
		(
			SELECT ROW_NUMBER() OVER (ORDER BY mem.[CPRO_ID] DESC) AS rownumber, mem.*
			FROM dbo.CVD_PRODUCT mem
			WHERE CPRO_DELETIONSTATE = 0
		) AS FOO
