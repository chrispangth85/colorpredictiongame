...
AS
	...
	BEGIN
		...
		BEGIN
			UPDATE [dbo].[CVD_USER]
			SET MEMBER_DOWNLINE_TOTAL_COMMISSION = MEMBER_DOWNLINE_TOTAL_COMMISSION + @sales
			WHERE [CUSR_USERNAME] = @intro
		END
		ELSE IF @cashName = 'UpdateTotalSponsor'
		BEGIN
			UPDATE [dbo].[CVD_USER]
			SET [MEMBER_TOTAL_DOWNLINE] = [MEMBER_TOTAL_DOWNLINE] + 1
			WHERE [CUSR_USERNAME] = @intro
		END
		ELSE IF @cashName = 'Recharge'
		BEGIN
			UPDATE [dbo].[CVD_USER]
			SET MEMBER_DOWNLINE_TOTAL_RECHARGE = MEMBER_DOWNLINE_TOTAL_RECHARGE + 1
			WHERE [CUSR_USERNAME] = @intro
		END
		ELSE IF @cashName = 'WDR'
		BEGIN
			UPDATE [dbo].[CVD_USER]
			SET [MEMBER_DOWNLINE_TOTAL_WITHDRAWAL] = [MEMBER_DOWNLINE_TOTAL_WITHDRAWAL] + 1
			WHERE [CUSR_USERNAME] = @intro
		END
