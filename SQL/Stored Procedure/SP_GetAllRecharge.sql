...
/****** Object:  StoredProcedure [dbo].[SP_GetAllRecharge]    Script Date: 19/3/2023 11:38:59 AM ******/
DROP PROCEDURE [dbo].[SP_GetAllRecharge]
GO
...
CREATE PROCEDURE [dbo].[SP_GetAllRecharge]
(
	@viewPage	INT,
...
AS
	...
	BEGIN
		...
		FROM dbo.CVD_CASHWALLETLOGTEMP
		WHERE CCASH_DELETIONSTATE = 0
	END
	ELSE IF @filterType = 'Name' AND @keyword != ''
	BEGIN
		SELECT @pages = COUNT(CCASH_ID)
		FROM dbo.CVD_CASHWALLETLOGTEMP mem, CVD_USER usr
